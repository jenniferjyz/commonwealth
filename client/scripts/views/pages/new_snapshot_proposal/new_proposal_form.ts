...
export const NewProposalForm: m.Component<{}, {
  ...
  view: (vnode) => {
    ...
    const members = vnode.state.space.members.map(address => address.toLowerCase());
    const isMember = author && author.address && members.includes(author.address.toLowerCase());
